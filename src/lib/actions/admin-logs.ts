...
export async function getLogsForAdmin(filters: AdminLogFilters) {
  ...
  return withRLS(session.userId, session.role, async (tx: DB) => {
    ...
    const [rows, [{ total }]] = await Promise.all([
      ...
      tx.select({ total: sql<number>`count(*)` }).from(dailyLogs).where(where),
    ]);
...
export async function updateLogByAdmin(
  ...
  const affectedPayrolls = await withRLS(session.userId, session.role, async (tx: DB) => {
    ...
    await logAudit(tx, "update", "daily_logs", logId, session.userId, before as Record<string, unknown>, safePatch);
